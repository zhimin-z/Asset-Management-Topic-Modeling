...
level = 2
model_name = 'title_r74ffrzc'
model_type = '_'.join(model_name.split('_')[:-1])

column_title = f'Challenge_preprocessed_title{level}'
column_content = f'Challenge_preprocessed_content{level}'
...
df = df[df.columns.drop(list(df.filter(regex=r'preprocessed')))]
