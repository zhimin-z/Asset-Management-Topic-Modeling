...
def _train():
    with wandb.init() as run:
        ...
        regressor = xgb.XGBRegressor(objective=run.config.objective, max_depth=run.config.max_depth, n_estimators=wandb.config.n_estimators, eta=wandb.config.eta)
...
wandb.agent(sweep_id, function=_train, count=500)
